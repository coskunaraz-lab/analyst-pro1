        ...
        try:
            ...
            with urlopen(req2, timeout=10) as r:
                ...
                for topic in (ddg.get('RelatedTopics') or [])[:5]:
                    text = topic.get('Text', '')
                    if text:
                        news_items.append({'title': text, 'source': 'DuckDuckGo', 'time': 0, 'url': topic.get('FirstURL', '')})
        except Exception as e:
            print(f'DDG error: {e}')
    return news_items
...
def fetch_ohlcv(ticker, period='2y'):
    ...
    closes = q['close']; opens = q['open']; highs = q['high']; lows = q['low']; volumes = q['volume']
    bars = []
    for i in range(len(timestamps)):
        if closes[i] is None: continue
        bars.append({'t': timestamps[i], 'o': opens[i], 'h': highs[i], 'l': lows[i], 'c': closes[i], 'v': volumes[i] or 0})
    return bars, meta
...
def macd_series(closes, fast=12, slow=26, signal=9):
    ...
    if len(valid) >= signal:
        idxs = [x[0] for x in valid]; vals = [x[1] for x in valid]
        sig = ema(vals, signal)
...
def bollinger(closes, n=20, k=2):
    upper = [None]*len(closes); lower = [None]*len(closes); mid = sma(closes, n)
    for i in range(n-1, len(closes)):
        sl = closes[i-n+1:i+1]; mn = sum(sl)/n
        sd = (sum((x-mn)**2 for x in sl)/n)**0.5
        upper[i] = mn + k*sd; lower[i] = mn - k*sd
...
def run_backtest(bars, strategy, params):
    ...
    if strategy == 'sma_cross':
        fast = sma(closes, int(params.get('fast', 20))); slow = sma(closes, int(params.get('slow', 50)))
        for i in range(1, n):
            if any(x is None for x in [fast[i], slow[i], fast[i-1], slow[i-1]]): continue
            if position is None and fast[i-1] <= slow[i-1] and fast[i] > slow[i]:
                ...
                trades.append({'entry': position['entry'], 'exit': closes[i], 'pnl': (closes[i]-position['entry'])/position['entry'], 'bars': i-position['idx']}); position = None
    elif strategy == 'ema_cross':
        fast = ema(closes, int(params.get('fast', 12))); slow = ema(closes, int(params.get('slow', 26)))
        for i in range(1, n):
            if any(x is None for x in [fast[i], slow[i], fast[i-1], slow[i-1]]): continue
            if position is None and fast[i-1] <= slow[i-1] and fast[i] > slow[i]:
                position = {'entry': closes[i], 'idx': i}
        ...
        rsi = rsi_series(closes, int(params.get('rsi_period', 14)))
        ob = float(params.get('overbought', 70)); os_lvl = float(params.get('oversold', 30))
        for i in range(1, n):
            if rsi[i] is None or rsi[i-1] is None: continue
            if position is None and rsi[i-1] < os_lvl and rsi[i] >= os_lvl: position = {'entry': closes[i], 'idx': i}
            elif position is not None and rsi[i-1] > ob and rsi[i] <= ob:
                trades.append({'entry': position['entry'], 'exit': closes[i], 'pnl': (closes[i]-position['entry'])/position['entry'], 'bars': i-position['idx']}); position = None
    elif strategy == 'macd_cross':
        macd_line, signal_line = macd_series(closes)
        for i in range(1, n):
            if any(x is None for x in [macd_line[i], signal_line[i], macd_line[i-1], signal_line[i-1]]): continue
            if position is None and macd_line[i-1] <= signal_line[i-1] and macd_line[i] > signal_line[i]: position = {'entry': closes[i], 'idx': i}
            elif position is not None and macd_line[i-1] >= signal_line[i-1] and macd_line[i] < signal_line[i]:
                trades.append({'entry': position['entry'], 'exit': closes[i], 'pnl': (closes[i]-position['entry'])/position['entry'], 'bars': i-position['idx']}); position = None
    elif strategy == 'bollinger_bounce':
        upper, lower, mid = bollinger(closes, int(params.get('bb_period', 20)), float(params.get('bb_std', 2.0)))
        for i in range(1, n):
            if lower[i] is None or mid[i] is None: continue
            if position is None and closes[i-1] < lower[i-1] and closes[i] > lower[i]: position = {'entry': closes[i], 'idx': i}
            elif position is not None and closes[i] >= mid[i]:
                trades.append({'entry': position['entry'], 'exit': closes[i], 'pnl': (closes[i]-position['entry'])/position['entry'], 'bars': i-position['idx']}); position = None
    elif strategy == 'rsi_divergence':
        rsi = rsi_series(closes, int(params.get('rsi_period', 14))); lb = int(params.get('lookback', 5))
        for i in range(lb, n-1):
            if any(rsi[j] is None for j in range(i-lb, i+1)): continue
            price_low = min(closes[i-lb:i+1]); rsi_low = min(rsi[i-lb:i+1])
            if closes[i] <= price_low and rsi[i] > rsi_low + 3 and position is None: position = {'entry': closes[i], 'idx': i}
            elif position is not None and rsi[i] > 60:
                trades.append({'entry': position['entry'], 'exit': closes[i], 'pnl': (closes[i]-position['entry'])/position['entry'], 'bars': i-position['idx']}); position = None
    if position is not None:
        trades.append({'entry': position['entry'], 'exit': closes[-1], 'pnl': (closes[-1]-position['entry'])/position['entry'], 'bars': n-position['idx'], 'open': True})
    if not trades: return {'trades': [], 'metrics': {}, 'equity': []}
    wins = [t for t in trades if t['pnl'] > 0]; losses = [t for t in trades if t['pnl'] <= 0]
    total_gain = sum(t['pnl'] for t in wins); total_loss = abs(sum(t['pnl'] for t in losses))
    equity = 1.0; peak = 1.0; max_dd = 0.0; eq_curve = [1.0]
    for t in trades:
        equity *= (1 + t['pnl']); eq_curve.append(round(equity, 4))
    ...
    metrics = {
        ...
        'bah_return': round((closes[-1]-closes[0])/closes[0]*100, 1),
        'avg_win': round(sum(t['pnl'] for t in wins)/len(wins)*100, 2) if wins else 0,
        'avg_loss': round(sum(t['pnl'] for t in losses)/len(losses)*100, 2) if losses else 0,
    ...
    recent = [{'entry': round(t['entry'],2), 'exit': round(t['exit'],2), 'pnl': round(t['pnl']*100,2), 'bars': t['bars']} for t in trades[-10:]]
...
class Handler(BaseHTTPRequestHandler):
    ...
    def send_json(self, data, status=200):
        ...
        self.cors_headers(); self.end_headers(); self.wfile.write(body)
    def send_file(self, path):
        ext = os.path.splitext(path)[1].lstrip('.')
        ct = {'html':'text/html','css':'text/css','js':'application/javascript','ico':'image/x-icon'}.get(ext,'text/plain')
        try:
    ...
    def do_GET(self):
        ...
        if path in ('/', '/index.html'):
            self.send_file(os.path.join(SCRIPT_DIR, 'index.html')); return
        if path == '/api/quote':
        ...
        if path == '/api/news':
            query = qs.get('q', [''])[0]
            if not query: self.send_json({'error': 'q gerekli'}, 400); return
            try: self.send_json({'news': fetch_news(query)})
            except Exception as e: self.send_json({'error': str(e)}, 500)
            return
        if path == '/api/backtest':
            ...
            try: params = json.loads(qs.get('params', ['{}'])[0])
            except: params = {}
            if not ticker: self.send_json({'error': 'ticker gerekli'}, 400); return
            try:
                bars, meta = fetch_ohlcv(ticker, period)
                result = run_backtest(bars, strategy, params)
                result['meta'] = {'ticker': ticker, 'strategy': strategy, 'period': period, 'bars_total': len(bars), 'currency': meta.get('currency','USD'), 'name': meta.get('longName', ticker)}
    ...
    def do_POST(self):
        ...
        except: self.send_json({'error': 'Gecersiz JSON'}, 400); return

        if self.path == '/api/claude':
            if not API_KEY: self.send_json({'error': 'API key eksik'}, 401); return
            prompt = payload.get('prompt', '')
            if not prompt: self.send_json({'error': 'prompt gerekli'}, 400); return
            try: self.send_json({'result': call_claude(prompt)})
            except Exception as e: self.send_json({'error': str(e)}, 500)
            return
        ...
        if self.path == '/api/trading_plan':
            ...
            if not market: self.send_json({'error': 'market gerekli'}, 400); return
